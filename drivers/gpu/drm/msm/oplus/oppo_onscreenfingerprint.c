...
int dsi_panel_parse_oppo_config(struct dsi_panel *panel)
{
	...
	panel->oppo_priv.lcd_cabc_support = utils->read_bool(utils->data,  "oplus,lcd-cabc-support");
	DSI_INFO("oplus,lcd-cabc-support: %s", panel->oppo_priv.lcd_cabc_support ? "true" : "false");
	panel->oppo_priv.is_lcd_type = utils->read_bool(utils->data, "oplus,is_lcd_type");
	DSI_INFO("is_lcd_type: %s", panel->oppo_priv.is_lcd_type ? "true" : "false");
...
********************************************/
	if (is_project(20085)) {  /* oled capacitive fingerprint project */
		panel->oppo_priv.fp_type = BIT(1);
	} else {
		panel->oppo_priv.fp_type = BIT(2);
	}
	pr_err("fp_type=0x%x", panel->oppo_priv.fp_type);
	return 0;
}
...
int oppo_ofp_set_fp_type(void *buf)
{
	...
	display->panel->oppo_priv.fp_type = *fp_type;
	return 0;
}
...
int oppo_ofp_get_fp_type(void *buf)
{
	...
	*fp_type = display->panel->oppo_priv.fp_type;
	DSI_INFO("fp_type:%d\n", *fp_type);
				...
				struct device_attribute *attr, char *buf)
{
	...
	DSI_INFO("fp_type:%d\n", display->panel->oppo_priv.fp_type);
