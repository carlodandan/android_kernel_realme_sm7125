...
static int oplus_chg_track_obtain_general_info(u8 *curx, int index, int len)
{
	...
	index += snprintf(&(curx[index]), len - index,
		...
		0, 0, 0, 0, 0, 0,					/* voocphy is not support in 4350 baseline; 
...
int oplus_chg_track_check_wired_charging_break(int vbus_rising)
{
	...
	pr_info("oplus_chg_track_check_wired_charging_break!!!");
...
EXPORT_SYMBOL(oplus_chg_track_obtain_power_info);
...
module_init(oplus_chg_track_driver_init);
module_exit(oplus_chg_track_driver_exit);
#else
